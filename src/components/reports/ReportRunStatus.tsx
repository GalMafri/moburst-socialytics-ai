...
export function ReportRunStatus({ reportId, startedAt, kind, onStarted }: {
  ...
  return <div className="space-y-4" role="status">
    ...
    {startedAt && <p className="t-secondary">Elapsed: {Math.floor(minutesRunning(report, now))} minutes. This page updates when the report is ready.</p>}
    {reportId && canRetry(report, now) ? <>
      <p className="t-secondary">This run has exceeded 90 minutes. It may still be working. Check its status before starting it again.</p>
      <RetryReportButton reportId={reportId} kind={kind} label="Review retry" onStarted={onStarted} />
    </> : <p className="t-secondary">Retry is unavailable for the first 90 minutes to prevent duplicate analyses.</p>}
  </div>;
}
