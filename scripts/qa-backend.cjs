...
function moduleFrom(rel,deps={},globals={},transform=x=>x){
 ...
 const exports={}; const context={exports,module:{exports},console:silent,setTimeout,DOMException,Response,Request,Headers,URL,URLSearchParams,Date,Error,Number,JSON,Set,Map,fetch:()=>{throw Error('Outbound network forbidden')},...globals,require:name=>{if(name in deps)return deps[name];if(name.endsWith('/reports/competitiveRange.ts'))return moduleFrom('supabase/functions/_shared/reports/competitiveRange.ts',{},globals);if(name.endsWith('/competitive/reportEvidence.ts'))return moduleFrom('supabase/functions/_shared/competitive/reportEvidence.ts',{},globals);if(name==='../_shared/competitive/fetchWithRateLimitRetry.ts')return moduleFrom('supabase/functions/_shared/competitive/fetchWithRateLimitRetry.ts',{},globals);if(name==='../_shared/competitive/rivaliqFetch.ts')return moduleFrom('supabase/functions/_shared/competitive/rivaliqFetch.ts',{},globals);throw Error('Unmocked dependency: '+name)}};
